    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _BulletinTab(service: _service, commissions: _commissions),
        ],
      ),
    );
  }
...
class _EmployesTab extends StatelessWidget {
  ...
  void _showFormulaireEmploye(BuildContext context, {Laveur? laveur}) {
    final nomCtrl = TextEditingController(text: laveur?.nom ?? '');
    final prenomCtrl = TextEditingController(text: laveur?.prenom ?? '');
    final telCtrl = TextEditingController(text: laveur?.telephone ?? '');
    final adresseCtrl = TextEditingController(text: laveur?.adresse ?? '');
    final cniCtrl = TextEditingController(text: laveur?.numeroCNI ?? '');
    DateTime dateNaissance = laveur?.dateNaissance ?? DateTime(1990, 1, 1);
        ...
        builder: (context, setModalState) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(
                  estModification ? 'Modifier l\'employé' : 'Nouvel employé',
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    if (picked != null) {
                      setModalState(() => dateNaissance = picked);
                    }
                  },
                  child: Container(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (estModification) {
                        await service.modifierLaveur(laveur.id, {
...
class _EmployeCard extends StatelessWidget {
  ...
  void _confirmerSuppression(BuildContext context) async {
    ...
    if (confirm == true) await service.supprimerLaveur(laveur.id);
  }
...
class _BulletinTab extends StatefulWidget {
  ...
  const _BulletinTab({required this.service, required this.commissions});
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Laveur>>(
      ...
      builder: (context, snapLaveurs) {
        ...
        if (laveurs.isNotEmpty && _laveurSelectionne == null) {
        ...
        return StreamBuilder<List<Lavage>>(
          ...
          builder: (context, snapLavages) {
            if (snapLavages.connectionState == ConnectionState.waiting) {
              return const Center(child: CircularProgressIndicator());
            }
            ...
            final lavagesFiltres = tousLavages.where((l) =>
                ...
                l.dateHeure.isBefore(
                    _dateFin.add(const Duration(seconds: 1)))).toList();

            final totalLavages = lavagesFiltres.length;
            final chiffreAffaires =
                lavagesFiltres.fold(0, (sum, l) => sum + l.prix);
            ...
            for (final l in lavagesFiltres) {
              ...
              commissionParService[l.service] =
                  (commissionParService[l.service] ?? 0) +
                ...
                children: [
                    ...
                    SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, i) {
                          final selected =
                              _laveurSelectionne == laveurs[i].nomComplet;
                  ...
                  if (parService.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Column(
                        children: widget.commissions.keys.map((service) {
                          final nb = parService[service] ?? 0;
                          final comm = commissionParService[service] ?? 0;
                          final taux = ((widget.commissions[service] ?? 0) *
                                  100)
                              .toInt();
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Divider(height: 24),
                        ...commissionParService.entries.map((e) =>
                            _BulletinRow(
                              label:
